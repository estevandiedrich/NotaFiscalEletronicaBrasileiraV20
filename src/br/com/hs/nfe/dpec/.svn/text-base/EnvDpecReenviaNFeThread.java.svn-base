...
public class EnvDpecReenviaNFeThread extends ThreadBase{
	private static final Logger logger = Logger.getLogger("EnvDpecReenviaNFeThread");
	...
	public EnvDpecReenviaNFeThread(Estabelecimento config)
	{
	...
	public void run()
	{
		Thread.currentThread().setName( "EnvDpecReenviaNFeThread_"+config.getCnpj()+"_"+config.getPe() );
		logger.info("Iniciando thread EnvDpecReenviaNFeThread_"+config.getCnpj()+"_"+config.getPe() );
		while ( super.isRunning() ) 
		{
			try
			{
				List<Nfe> nfeList = nfeDao.procuraNotasReenviar(config.getCnpj(),config.getPe());
				for(Nfe nfe:nfeList)
				{
					NFeVO enviNFeXML = DpecEmailsEnviados.getInstance().getNFeVOPorChaveAcesso(nfe.getChaveAcesso());
					if(enviNFeXML == null)
					{
						try
						{
